...
# tehát a portás 10:50-kor zárta be a kaput, így ezek a srácok 10:45 - 10:50-ig hagyhatták el a helyszínt, és nem csekkolták le magukat, tehát amikor visszajöttek 10:50 - 11:00 között, akkor ha a 4, 3 lehúzót (menza, könyvtár) nem vesszük figyelembe, akkor a kártyájukon kétszer léptek be, ezeket keressük. 1, 1, 2, 1, 2
# ötlet: szedjük össze az összes ilyen embernek (16 ember) napi tevékenységét, ne vegyük figyelembe a 3, 4, és ha egymás után kapunk két 1-est, akkor elkaptuk a delikvenst.
...
# Létrehozunk egy üres listát(belepes_kilepes). végigfutunk a 10:50 - 11:00 bejövő diákok azonosítóin ('EQBL') ez esetünkben a tanulo == 'EQBL', ezzel kezdhetünk valamit. minden egyes tanuló azonosítójánál végigfutunk a teljes listán (beleptetes) előtte létrehozunk egy üres listát (belso_lista), ez arra szolgál, hogy ebbe tegyük bele a kilepes, és belépés esemény kódját, majd vizsgálódunk, hogy ha a ( tanulo == nagy listánk (beleptetes) azonositójával ) ÉS ( 1-es, vagy 2-es kód van az egyelemen belül ), akkor beletesszük ebbe a belso_lista-ba. Majd ha végigment ez a beleptetes lista, tehát az összetett ciklusunk második ciklusa (amiben a nagy beleptetes listát futjuk át), ez esetben a belso_lista-nk kész van, appendelhetjuk a külsőbe -> belepes-kilepes.
